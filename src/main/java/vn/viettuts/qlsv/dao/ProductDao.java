...
import vn.viettuts.qlsv.entity.XML.ProductXML;
...
public class ProductDao {
    ...
    private boolean isExistProduct(String id) {
    ...
    public void addProduct(Product product) throws IllegalArgumentException {
        if (isExistProduct(product.getId())) {
            throw new IllegalArgumentException("Mã sản phẩm đã tồn tại.");
        }

        if (!checkExpiredDate(product.getProductExpiredDate())) {
    ...
    public void updateProduct(String oldId, Product newProduct) throws IllegalArgumentException {
        ...
        if (!checkExpiredDate(newProduct.getProductExpiredDate())) {
            throw new IllegalArgumentException("Hạn sử dụng phải lơn hơn ngày hiện tại.");
        }
    ...
    public List<Product> search(String keyword) {
        ...
        for (Product product : products) {
            if (product.getId().toLowerCase().contains(keyword.toLowerCase()) ||
                    ...
                    product.getProductCategory().toLowerCase().contains(keyword.toLowerCase()) ||
                    product.getProductSupplier().toLowerCase().contains(keyword.toLowerCase()) ||
                    product.getPrice().toLowerCase().contains(keyword.toLowerCase())) {
                result.add(product);
            }
        }
